...
export async function provisionAgent(opts: ProvisionAgentOptions): Promise<AgentDoc & { agentToken: string }> {
  ...
  const commons = await registerWithAgentCommons(agentId, opts.role, opts.systemPrompt)
...
async function registerWithAgentCommons(
  ...
  try {
    ...
    const data = await res.json() as { id?: string; apiKey?: string; walletAddress?: string }
