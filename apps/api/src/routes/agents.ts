...
import { getCloudProvider } from '@commonos/cloud'
...
router.post('/:id/agents', async (c) => {
  ...
  const fleet = await (await fleets()).findOne({ _id: fleetId, tenantId: c.get('tenantId') })
  ...
  try {
    ...
    return c.json({ error: err instanceof Error ? err.message : 'provisioning failed' }, 503)
  }
})
...
router.patch('/:id/agents/:agentId', async (c) => {
  ...
  try {
    await (await agents()).updateOne(
      { _id: c.req.param('agentId'), fleetId: c.req.param('id'), tenantId: c.get('tenantId') },
      { $set: update },
    )
...
router.delete('/:id/agents/:agentId', async (c) => {
  ...
  try {
    ...
    await col.updateOne({ _id: agent._id }, { $set: { status: 'terminated', updatedAt: new Date() } })
