...
const AGC_URL = (process.env.AGC_API_URL ?? "https://api.agentcommons.io").replace(/\/$/, "");
...
async function verifyAgentCommonsOwner(
	...
	if (!token) {
		return { response: c.json({ error: "Agent Commons ownership verification is unavailable" }, 503) };
	}
	...
	if (!response.ok) {
		return {
			response: c.json(
				{ error: "Agent Commons agent was not found or is not owned by this account" },
	...
	if (!ownerUserId) {
		return { response: c.json({ error: "Agent Commons agent has no canonical owner" }, 409) };
	}
	if (
		c.get("authType") !== "service" &&
		(c.get("userId") !== ownerUserId ||
			(c.get("workspaceId") && agent.workspaceId && c.get("workspaceId") !== agent.workspaceId))
	) {
		return { response: c.json({ error: "Agent Commons agent is not owned by this account" }, 403) };
	}
	return {
		owner: {
			ownerUserId,
			...
			const converted: AgentDoc = {
				...
				compute: {
					...
					currentActiveStartedAt:
						legacy.status === "stopped" ? null : legacy.startedAt ?? legacy.createdAt,
...
router.post("/", async (c) => {
	...
	if (!agentCommonsId) return c.json({ error: "agentCommonsId is required" }, 400);
	if (c.get("authType") === "service" && serviceBinding(c) !== agentCommonsId) {
		return c.json({ error: "service binding does not match agentCommonsId" }, 403);
	}
	...
	try {
		...
		const fleet = await ensureTenantComputerFleet({ tenant, owner: verified.owner });
		let existing = await (await agents())
			.findOne({
				kind: "computer",
		...
		if (existing && existing.tenantId !== tenant._id) {
			await (await agents()).updateOne(
				{ _id: existing._id, kind: "computer", externalAgentId: agentCommonsId },
				{
					$set: {
						tenantId: tenant._id,
						fleetId: fleet._id,
		...
		if (existing) {
			...
			const changed =
				JSON.stringify(existing.resourceSpec) !== JSON.stringify(resourceSpec) ||
					...
					? {
							...
							idleTtlMinutes:
								body.idleTtlMinutes === undefined
									...
									: Math.max(5, Math.min(Number(body.idleTtlMinutes) || 60, 1440)),
							policy: body.policy
								? {
			...
			if (changed && ["running", "idle", "starting"].includes(existing.status)) {
		...
		const created = await provisionAgent({
			...
			idleTtlMinutes: Math.max(5, Math.min(Number(body.idleTtlMinutes) || 60, 1440)),
			computerPolicy: {
				allowBrowser: body.policy?.allowBrowser !== false,
				allowTerminal: body.policy?.allowTerminal !== false,
				allowFilesystem: body.policy?.allowFilesystem !== false,
		...
		return c.json(
			{ error: error instanceof Error ? error.message : "computer provisioning failed" },
			503,
		);
	}
});
...
router.get("/", async (c) => {
	...
	const list = await (await agents()).find(query).sort({ createdAt: -1 }).lean();
...
router.post("/:computerId/instructions", async (c) => {
	...
	const computer = await getBoundComputer(c);
	if (isResponse(computer)) return computer;
	if (!["running", "idle", "starting", "provisioning"].includes(computer.status)) {
		return c.json({ error: "computer is not running" }, 409);
	}
	let sessionId = body.sessionId ?? null;
	if (sessionId) {
		const session = await ensureRuntimeSessionForAgcSession(computer, sessionId, {
...
router.patch("/:computerId", async (c) => {
	...
	try {
		...
		if (
			...
			changed = JSON.stringify(computer.resourceSpec) !== JSON.stringify(resolved.spec);
			updated = {
				...computer,
				resourceProfile: resolved.profile,
		...
		return c.json({ error: error instanceof Error ? error.message : "computer update failed" }, 422);
	}
});
...
router.post("/:computerId/restart", async (c) => {
	...
	try {
		...
		return c.json({ error: error instanceof Error ? error.message : "computer restart failed" }, 503);
	}
});
...
router.delete("/:computerId", async (c) => {
	...
	try {
		...
		return c.json({ error: error instanceof Error ? error.message : "computer deletion failed" }, 503);
	}
});
