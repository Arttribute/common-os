...
async function createAgcSession(
...
async function ensureAxlSession(
  ...
  const label =
    participantAgentId ??
    (participantPeerId ? participantPeerId.slice(0, 16) : "unknown peer");
  ...
  const sessionId =
    agcSessionId ??
    `axlsess_${Date.now().toString(36)}${randomBytes(4).toString("hex")}`;
  const now = new Date();
...
async function resolveAgcSessionId(
  agentId: string,
  ...
  try {
    const sess = await (await agentSessions())
      .findOne(
...
router.get("/resolve/:name", async (c) => {
  ...
  try {
    ...
    const looksLikePeerId =
      /^(Qm[1-9A-HJ-NP-Za-km-z]{40,}|12D3[a-zA-Z0-9]{40,})/.test(name);
    ...
    return c.json({
      ...
      fleetId: agent.fleetId,
      walletAddress:
        agent.wallet?.address ?? agent.commons?.walletAddress ?? null,
...
router.post("/:agentId/wallet/send-transaction", async (c) => {
  ...
  try {
    ...
    if (!agent) return c.json({ error: "agent not found" }, 404);
    const tx = await sendAgentTransaction(
      agent,
...
router.get("/:agentId/tasks/next", async (c) => {
  ...
  if (!taskId) {
    ...
    try {
      const claimed = await (await tasks())
        .findOneAndUpdate(
...
router.post("/:agentId/tasks/:taskId/complete", async (c) => {
  ...
  const body = (await c.req
    .json<{ output?: string; error?: string }>()
    .catch(() => ({}))) as {
    output?: string;
    error?: string;
  };
...
router.get("/:agentId/messages/next", async (c) => {
  ...
  async function dispatchMessage(msg: HumanMessageDoc) {
    ...
    if (!agcSessionId) {
      console.warn(
        `[runtime] message ${msg._id} has no Agent Commons sessionId; daemon will use runtime-local session handling`
      );
    }
    ...
    console.log(
      `[runtime] dispatch message ${msg._id} agcSession=${
        agcSessionId ?? "none"
        ...
        .updateOne(
          {
            ...
            $or: [
              ...
              { agcSessionId: existing.sessionId },
            ],
          },
          { $set: { lastMessageAt: now } }
        )
      ...
      .findOneAndUpdate(
        { _id: msgId, agentId, status: { $ne: "responded" } },
        {
          $set: {
            ...
            respondedAt: now,
            updatedAt: now,
            ...(!existing?.sessionId && body.agcSessionId
              ? { sessionId: body.agcSessionId }
              : {}),
          },
        },
        { new: true }
    ...
    if (statsSessionId) {
      await (
        ...
        .updateOne(
          {
            agentId,
            $or: [{ _id: statsSessionId }, { agcSessionId: statsSessionId }],
          },
          { $inc: { messageCount: 1 }, $set: { lastMessageAt: now } }
        )
        .catch(() => {});
...
router.post("/:agentId/messages/:msgId/event", async (c) => {
  ...
  if (
    !["message_status", "message_delta", "tool_call", "tool_result"].includes(
      eventType
    )
  ) {
...
router.post("/:agentId/bootstrap", async (c) => {
  ...
  try {
    ...
    if (agent.commons.agentId || agent.commons.registryAgentId) {
      ...
      if (!commons.agentId) {
        return c.json(
          { error: "agent is missing Agent Commons identity" },
          409
        );
      }
...
router.post("/:agentId/session", async (c) => {
  ...
  const body = await c.req
    .json<{ agcSessionId: string; title?: string }>()
    .catch(() => ({ agcSessionId: "", title: undefined }));
  if (!body.agcSessionId)
    return c.json({ error: "agcSessionId is required" }, 400);
  ...
  try {
    ...
    return c.json(
      { sessionId: body.agcSessionId, agcSessionId: body.agcSessionId },
      201
    );
  } catch {
    return c.json({ error: "database error" }, 503);
