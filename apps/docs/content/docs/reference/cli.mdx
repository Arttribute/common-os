...
`--runtime` can be `native`, `openclaw`, `hermes`, or `guest`. `--image`
without `--runtime` keeps the legacy behavior and sets `integrationPath:
"guest"`, running the image as the
...
```bash
commonos computer create --agent-commons-id agent_xxx --profile standard --mode elastic
commonos computer ls --agent-commons-id agent_xxx
commonos computer wake computer_xxx
commonos computer sleep computer_xxx
commonos computer restart computer_xxx
commonos computer resize computer_xxx --profile performance
commonos computer resize computer_xxx --vcpu 4 --memory 8 --storage 50
```

`computer terminate` is an explicit destructive compatibility command: it
removes the computer and workspace. Use `computer sleep` when the workspace
must persist.
