...
| Goal | Where to start |
| --- | --- |
| Try the product locally | [Local setup](/docs/getting-started/local-setup) |
| Understand the mental model | [Core concepts](/docs/core-concepts/fleets) |
| Create fleets and agents | [Create a fleet](/docs/guides/create-a-fleet) |
| Send work to agents | [Send tasks](/docs/guides/send-tasks) |
| Use the CLI | [CLI reference](/docs/reference/cli) |
| Use the SDK | [SDK reference](/docs/reference/sdk) |
| Deploy the platform | [GCP deployment](/docs/operations/gcp-deployment) |
...
| `apps/runner` | Cloud Run service that wraps Agent Commons CLI execution. |
...
The repository has a full local and cloud-oriented implementation: Mongo-backed control plane, API key and agent token auth, GKE and EKS pod provisioners, Agent Commons registration hooks, wallet helpers, an agent daemon, a CLI, a TypeScript SDK, and a real-time world UI.

Some integrations depend on external credentials or live services. For example, real GKE provisioning requires GCP credentials, `MONGODB_URI`, an agent image, and `RUNNER_URL`. Without a live API connection, the World UI can still run in demo mode.
