...
| `config.integrationPath` | `native`, `openclaw`, or `guest`. |
...
| `guest` | Tenant-provided Docker image plus SDK | Custom frameworks such as LangGraph, CrewAI, AutoGen, or internal agents. |

## Provisioning Flow

1. API creates a CommonOS agent ID and `cos_agent_...` token.
2. Native agents attempt Agent Commons registration.
